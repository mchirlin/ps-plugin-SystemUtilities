...
public class RulesFolderRenamer extends AppianSmartService {

	private static final Logger LOG = Logger.getLogger(RulesFolderRenamer.class);
	...
	public void run() throws SmartServiceException {
		...
		try {
			...
			if (currentVersion.getType().equals(ContentConstants.TYPE_FOLDER)) {
				content = (Content[]) cs.getAllVersions(cs.getIdByUuid(constantUuid));
				for(Content c : content) {
					c.setName(newName);
					c.setDescription(newDescription);
				}
				cs.updateFields(content, new Integer[]{ContentConstants.COLUMN_NAME, ContentConstants.COLUMN_DESCRIPTION}, ContentConstants.UNIQUE_FOR_PARENT);
	...
	public RulesFolderRenamer(SmartServiceContext smartServiceCtx, ContentService cs) {
		super();
	...
	@Input(required = Required.ALWAYS)
	@Name("newDescription")
	public void setNewDescription(String val) {
