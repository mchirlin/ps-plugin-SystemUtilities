...
public class UpdateExpressionRule extends AppianSmartService {

	private static final Logger LOG = Logger.getLogger(UpdateExpressionRule.class);
	...
	public void run() throws SmartServiceException {
		...
		LOG.debug("The following rule will be updated: " + constantUuid);
		try {

			LOG.debug("Retrieving constant " + constantUuid);
			FreeformRule currentVersion = (FreeformRule) cs.getVersion(constantUuid, ContentConstants.VERSION_CURRENT);

			if (currentVersion.getType().equals(ContentConstants.TYPE_RULE)) {
				currentVersion.setDefinition(expression);
				cs.updateFields(currentVersion, new Integer[]{ContentConstants.COLUMN_ATTRIBUTES}, ContentConstants.UNIQUE_NONE);
				LOG.debug("Object " + currentVersion.getUuid() + " has been updated");
			}
			...
			LOG.error(e);
			throw createException(e, "error.invalidRecursionException");
	...
	public UpdateExpressionRule(SmartServiceContext smartServiceCtx, ContentService cs) {
		super();
